...
class ImageDataset(Dataset):
    ...
    def __init__(self, type='train', transform=None):
        ...
        mp = {}
        for i, img_lb in enumerate(sorted(os.listdir(cfg['img_base']))):
            mp[i] = img_lb
            for img_f in os.listdir(os.path.join(cfg['img_base'], img_lb)):
                files.append(os.path.join(cfg['img_base'], img_lb, img_f))
                types.append(i)
