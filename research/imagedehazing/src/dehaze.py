...
from research.imagedehazing.src.guidedfilter import guided_filter
...
def get_atmosphere(I, darkch, p):
    ...
    print('[INFO] Atmosphere light region:', [(i / N, i % N) for i in searchidx])
...
def dehaze_raw(I, tmin=0.2, Amax=220, w=15, p=0.0001,
    ...
    print('[INFO] Atmosphere', A)

    rawt = get_transmission(I, A, Idark, omega, w)
    print('[INFO] Raw transmission rate between [%.4f, %.4f]' % (rawt.min(), rawt.max()))
    ...
    print('[INFO] Refined transmission rate between [%.4f, %.4f]' % (refinedt.min(), refinedt.max()))
...
def dehaze(im, tmin=0.2, Amax=220, w=15, p=0.0001,
    ...
    def to_img(raw):
        ...
        if len(raw.shape) == 3:
            print('[INFO] Range for each channel:')
            for ch in range(3):
