...
from cv.models.skin_disease_recognizer import skin_disease_recognizer
...
def upload_and_rec_skin_disease(request):
    ...
    if request.method == "POST":
        ...
        if not isinstance(image, InMemoryUploadedFile) and not isinstance(image, TemporaryUploadedFile):
            ...
            image = 'cv/static/SkinUpload/' + image.name

        tik = time.time()
        skin_disease = skin_disease_recognizer.infer(image)
        result['code'] = 0
        result['msg'] = 'Success'
        result['imgpath'] = imagepath
        if max(_['probability'] for _ in skin_disease['results']) > cfg['thresholds']['skin_disease_recognition']:
